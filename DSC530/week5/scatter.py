...
def binned_percentile_for_age(df):
    ...
    thinkplot.Save(root='chap07_exercise_7_1_binned_percentile_plot',
                   formats=['png'],
                   xlabel="mother's age (years)",
                   ylabel='birth weight (lbs)')


def draw_scatter_plot(ages, weights, alpha=1.0):
...
def main(script):
    ...
    """Using scatter.py to complete exercise.
    Commented code from scatter.py which is not required for now. """
    ...
    binned_percentile_for_age(live)

    ages = live.agepreg

    weights = live.totalwgt_lb
    print(f'Pearson Correlation : {thinkstats2.Corr(ages, weights)}')
    print(f'Spearman Correlation:{thinkstats2.SpearmanCorr(ages, weights)}')

    draw_scatter_plot(ages, weights, alpha=0.1)
    thinkplot.Save(root='chap07_exercise_7_1_scatter_plot',
                   legend=False,
                   formats=['png'])
...
# Scatter plot is not showing relation between age and birth weight very clearly.
# Based on Pearson correlation around 0.07 and Spearman correlation around 0.09.
# The difference between Pearson and Spearman correlation shows the relationship is nonlinear.

# The percentile plot of mother's age  versus birth weight suggests that the
# relationship is non-linear.  From 15th to 25th percentile it shows linear and after
# 25th percentile there is not clear relation.
