...
def run_test_with_reduced_sample(live, iters=1000):
    """Using hypothesis.py used in chapter 09 for adding requried test."""
    n = len(live)
    print('-' * 50)
    ...
    # compare pregnancy lengths
    sample_values = firsts.prglngth.values, others.prglngth.values
    ht = DiffMeansPermute(sample_values)
    p_value = ht.PValue(iters=iters)
    print(f'Test with prglngth data: p-Value = {round(p_value,2)}')

    sample_values = (firsts.totalwgt_lb.dropna().values,
            others.totalwgt_lb.dropna().values)
    ht = DiffMeansPermute(sample_values)
    p_value = ht.PValue(iters=iters)
    print(f'Test with birth weight data: p-Value = {round(p_value,2)}')

    live2 = live.dropna(subset=['agepreg', 'totalwgt_lb'])
    sample_values = live2.agepreg.values, live2.totalwgt_lb.values
    ht = CorrelationPermute(sample_values)
    p_value = ht.PValue(iters=iters)
    print(f'Correlation Test: p-Value = {round(p_value,2)}')

    sample_values = firsts.prglngth.values, others.prglngth.values
    ht = PregLengthTest(sample_values)
    p_value = ht.PValue(iters=iters)
    print(f'Chi-Squared stats p-Value = {round(p_value,2)}')
...
def main():
    ...
    for _ in range(8):
        ...
        run_test_with_reduced_sample(sample)
        n = int(n/2)

    # Exercise 9.1 -
    # Test result shows that the reduced sample size causes results becomes negative. This looks like erratic data
    # because some of the test are showing positive test even with small sample size.
